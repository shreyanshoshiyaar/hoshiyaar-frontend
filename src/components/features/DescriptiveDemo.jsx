...
const QUESTIONS = [
  { id: 1, text: "How do we make something neither acidic or basic?", expected: "By mixing an acid and a base in the right amounts to neutralize each other." },
  { id: 2, text: "What is heat? Write one basic idea about how heat moves.", expected: "Heat is a form of energy. It moves from a hotter object to a colder object." },
  { id: 3, text: "Why do we sweat when we exercise?", expected: "We sweat to cool our bodies down. The evaporation of sweat removes heat from the skin." },
  { id: 4, text: "What is photosynthesis?", expected: "Photosynthesis is the process by which plants use sunlight, water, and carbon dioxide to create oxygen and energy in the form of sugar." },
  { id: 5, text: "Why does ice float on water?", expected: "Ice floats because it is less dense than liquid water, due to the way hydrogen bonds arrange water molecules into a crystal lattice." }
];
...
const ScoreGauge = ({ score }) => {
  ...
  const getColour = (value) => {
    if (value >= 80) return '#26E79A';
    if (value >= 50) return '#f5ce62';
    return '#dd776e';
  };
  const color = getColour(displayScore);

  const angle = (displayScore / 100) * 2 * Math.PI;
  const headX = Math.sin(angle) * normalizedRadius;
  const headY = -Math.cos(angle) * normalizedRadius;

  return (
    <div className="relative flex flex-col items-center justify-center shrink-0 mb-4 sm:mb-10 bg-white/10 backdrop-blur-md border border-white/10 rounded-[2rem] shadow-xl p-4 sm:p-6 w-36 h-36 sm:w-[220px] sm:h-[220px]">
      <svg viewBox={`0 0 ${radius * 2} ${radius * 2}`} className="w-full h-full relative z-10 overflow-visible">
        <g transform={`translate(${radius}, ${radius})`}>
          <circle r={radius + 18} stroke="#ffffff" strokeWidth="2" fill="none" opacity="0.1" />
          <circle r={radius + 5} stroke="#ffffff" strokeWidth="12" fill="none" opacity="0.05" />
          <circle r={normalizedRadius} stroke="rgba(255,255,255,0.1)" strokeWidth={stroke} fill="none" />
          <circle
            r={normalizedRadius}
            stroke={color}
            strokeWidth={stroke}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        <span className="text-4xl sm:text-6xl font-black drop-shadow-sm tracking-tighter" style={{ color }}>{displayScore}</span>
        <span className="text-[10px] sm:text-xs font-bold text-white/60 mt-0 sm:mt-1 uppercase tracking-[0.2em]">Score</span>
      </div>
    </div>
  );
};

const DescriptiveDemo = () => {
  ...
  const [attempts, setAttempts] = useState(Array(QUESTIONS.length).fill(1));
  ...
  const submitBatchQuiz = async () => {
    ...
    try {
      ...
      QUESTIONS.forEach((q, i) => {
        if (!answers[i].trim()) {
          localFeedbacks[i] = { id: q.id, score: 0, isCorrect: false, missing: "You skipped this question.", wrong: "No answer provided.", grammar: null };
        } else {
          itemsToEvaluate.push({ id: q.id, index: i, question: q.text, expectedAnswer: q.expected, userAnswer: answers[i] });
        }
      });

      if (itemsToEvaluate.length > 0) {
        const res = await api.post('/api/ai/evaluate-batch', { items: itemsToEvaluate });
  ...
  const TopBar = ({ title, onBack }) => (
    <div className="flex items-center justify-center p-4 sm:px-6 text-white shrink-0 max-w-3xl mx-auto w-full relative h-20">
      ...
      <h1 className="text-xl sm:text-xl font-black tracking-widest uppercase flex items-center gap-2 z-10 text-center px-12">
    ...
    <div className="px-4 py-2 sm:px-6 flex items-center gap-2 shrink-0 max-w-3xl mx-auto w-full mt-2">
      <div className="flex-1 flex items-center justify-between relative">
        ...
        <div 
          ...
          style={{ width: `${(currentIdx / (QUESTIONS.length - 1)) * 100}%` }}
        ></div>
        {QUESTIONS.map((_, i) => (
      ...
      <span className="text-white font-bold ml-6 sm:text-base">{currentIdx + 1}/{QUESTIONS.length}</span>
    </div>
  );
    ...
    >
      ...
      {screen === 'CHALLENGE' && (
        <div className="flex flex-col h-full relative z-10 animate-fade-in min-h-0 w-full">
          ...
          <TopBar title={mode === 'REVIEW' ? "Exam Analysis" : (mode === 'QUIZ' ? "Exam AI Mode" : "Retry Question")} onBack={() => mode === 'REVIEW' ? setScreen('REPORT') : (mode === 'QUIZ' ? navigate(-1) : setScreen('REPORT'))} />
          ...
          <div className="flex-1 p-2 sm:px-4 sm:py-3 flex flex-col gap-3 overflow-y-auto min-h-0 max-w-5xl mx-auto w-full">
            ...
            {mode !== 'REVIEW' && (
               ...
               >
                 {mode === 'QUIZ' ? (currentIdx < QUESTIONS.length - 1 ? 'Next Question' : 'Submit Quiz') : 'Submit Retry'}
               </button>
            )}
          </div>
        </div>
              ...
              <div className="flex-1 w-full flex flex-col bg-[#1A2C5B] rounded-3xl p-4 md:p-5 shadow-2xl self-stretch border border-white/5 md:scale-95">
                ...
                <div className="flex flex-col gap-2 flex-1 justify-center">
                  {QUESTIONS.map((q, idx) => {
                    ...
                    return (
                      <div key={q.id} className="bg-[#263B69] border border-white/5 p-2.5 md:p-3 rounded-xl flex items-center justify-between text-left shrink-0 shadow-sm hover:bg-[#2C447A] transition-colors">
...
export default DescriptiveDemo;
