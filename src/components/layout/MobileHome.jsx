      ...
      <div className="relative z-10 w-full flex flex-col px-4 pt-4">
        ...
        <div className="w-full bg-white/95 rounded-[20px] p-2 shadow-[0_4px_12px_rgba(0,0,0,0.03)] flex justify-between items-center border border-white/60 backdrop-blur-md mb-2">
          ...
          <div className="flex items-center gap-1.5 w-1/3 justify-center">
            ...
            <div className="flex flex-col">
              <span className="text-[13px] font-bold text-gray-400 uppercase tracking-wider">Rank</span>
              <span className="text-[24px] font-black text-gray-800 leading-none">{myRank}</span>
              <div className="w-8 h-0.5 rounded-full bg-purple-500 mt-0.5"></div>
            </div>
          </div>
          ...
          {/* Stars */}
          <div className="flex items-center gap-1.5 w-1/3 justify-center">
            <div className="w-9 h-9 bg-yellow-100 rounded-full flex items-center justify-center text-[20px] drop-shadow-sm">⭐</div>
            <div className="flex flex-col">
              <span className="text-[13px] font-bold text-gray-400 uppercase tracking-wider">Stars</span>
              <span className="text-[24px] font-black text-gray-800 leading-none">{stars}</span>
            </div>
          </div>
          ...
          {/* Streak */}
          <div className="flex items-center gap-1.5 w-1/3 justify-center">
            <div className="w-9 h-9 bg-orange-100 rounded-full flex items-center justify-center text-[20px] drop-shadow-sm">🔥</div>
            <div className="flex flex-col">
              <span className="text-[13px] font-bold text-gray-400 uppercase tracking-wider">Streak</span>
              <div className="flex items-baseline gap-1">
                <span className="text-[24px] font-black text-gray-800 leading-none">{myStreak}</span>
                <span className="text-[13px] font-bold text-gray-500">Days</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-2 mb-3">
          ...
          <div className="col-span-7 bg-[#3B82F6] rounded-[24px] p-1 overflow-hidden relative shadow-md flex flex-col min-h-[180px] border border-blue-400">
            ...
            <div className="bg-white rounded-[20px] p-3 flex-1 flex flex-col items-center justify-center relative shadow-inner">
              {/* Panel space blank per request */}
              <div className="flex flex-col items-center opacity-10">
                <div className="text-5xl mb-3">🚀</div>
                <div className="text-[11px] font-black uppercase tracking-widest text-blue-900">Next Mission Loading...</div>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-5 flex flex-col gap-3 min-h-[180px]">
            ...
            <div className="bg-[#F9F8FF] rounded-[24px] p-4 shadow-sm border border-[#E9D8FF] flex-1 flex flex-col items-center justify-center relative overflow-hidden">
              <div className="absolute top-2 left-2 text-[#C084FC] text-xl opacity-40">✦</div>
              <div className="absolute top-4 right-3 text-yellow-400 text-lg opacity-40">✦</div>

              <div className="relative mb-2">
                <div className="text-[44px] drop-shadow-md">📅</div>
                <div className="absolute -bottom-1 -right-2 text-2xl drop-shadow-md">⭐</div>
              </div>

              <p className="text-[#4A5568] font-bold text-center text-sm leading-tight">
                Fresh mysteries <br />every
              </p>

              <div className="flex items-center gap-1 mt-1 text-[#8B5CF6] font-black text-lg">
                <span className="opacity-50 text-xl">≽</span> Tues & Fri <span className="opacity-50 text-xl">≼</span>
              </div>
            </div>

            {/* Social Card (Exact Match) */}
            <div className="bg-[#EBF5FF] rounded-[24px] p-4 shadow-sm border border-[#D6E4FF] flex-1 flex flex-col items-center justify-center">
              <div className="flex items-center gap-2 mb-4 bg-white/50 px-4 py-1.5 rounded-full border border-white/40 shadow-sm">
                <h3 className="text-[#2C4A86] font-black text-[13px] whitespace-nowrap">Stay Connected</h3>
              ...
              <div className="flex gap-2.5 w-full">
                <a href="https://www.instagram.com/hoshiyaar_club/" target="_blank" rel="noopener noreferrer" className="flex-1 bg-white rounded-2xl p-2 flex flex-col items-center justify-center shadow-md border border-white hover:bg-gray-50 transition-all">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white mb-1 shadow-sm" style={{ background: 'linear-gradient(45deg, #f09433 0%, #dc2743 50%, #bc1888 100%)' }}>
                ...
                <a href="https://www.youtube.com/@Hoshi-yaar" target="_blank" rel="noopener noreferrer" className="flex-1 bg-white rounded-2xl p-2 flex flex-col items-center justify-center shadow-md border border-white hover:bg-gray-50 transition-all">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-sm bg-[#FF0000] mb-1">
        ...
        <div className="w-full bg-[#F8FAFF] rounded-[24px] p-3 pt-3 pb-4 mb-2 shadow-inner border border-white/40 relative overflow-hidden">
          <div className="flex items-center justify-center gap-1.5 mb-2">
            <span className="text-yellow-400 text-[14px]">⭐</span>
            <h2 className="text-[19px] font-black text-gray-800 tracking-tight">Weekly Leaderboard</h2>
            <span className="text-yellow-400 text-[14px]">⭐</span>
          </div>

          <div className={`flex gap-2 w-full justify-center transition-all duration-500 ${!hasSchool ? 'blur-md grayscale opacity-50 select-none' : ''}`}>
            {lbItems.length > 0 ? (
              lbItems.map((item, idx) => (
                <div
                  key={idx}
                  className={`flex-1 min-w-[80px] rounded-[20px] p-2.5 pt-4 pb-2 flex flex-col items-center justify-center relative shadow-md border ${item.isMe ? 'border-[#2563EB] border-2 bg-white' : 'border-gray-100 bg-white'
                    }`}
                >
                  {/* Rank Badge */}
                  <div className={`absolute -top-1.5 -left-1.5 w-7 h-7 rounded-full text-white font-black flex items-center justify-center text-[14px] shadow-lg ${item.isMe ? 'bg-[#2563EB]' : 'bg-[#7C3AED]'
                    }`}>
                    {item.rank}
                  </div>

                  <div className={`font-black text-[16px] truncate w-full text-center mb-1 ${item.isMe ? 'text-[#1E40AF]' : 'text-gray-900'
                    }`}>
                    {item.isMe ? 'You' : (item.username || '—')}
                  </div>

                  <div className="flex items-center gap-0.5 bg-white rounded-full px-2 py-0.5 shadow-[inset_0_1px_2px_rgba(0,0,0,0.06)] border border-gray-50">
                    <span className="text-sm drop-shadow-sm">⭐</span>
                    <span className="text-[14px] font-black text-gray-700">{item.totalPoints || 0}</span>
                  </div>
                </div>
              ))
            ) : (
              <div className="flex flex-col items-center py-4 opacity-40">
                <span className="text-xs font-black text-blue-900 uppercase">No Rankings yet</span>
              </div>
            )}
          </div>
        ...
        <div className="fixed bottom-[80px] left-0 right-0 px-5 z-[100] pb-2 pt-3 bg-gradient-to-t from-[#F0F6FF] via-[#F0F6FF]/90 to-transparent pointer-events-none">
          <button
            ...
            className="w-full py-3 bg-[#FFC107] hover:bg-[#FFD54F] text-gray-900 rounded-full font-black text-[22px] shadow-[0_4px_0_0_#F57F17] active:translate-y-1 active:shadow-[0_0px_0_0_#F57F17] flex items-center justify-center gap-2 transition-all pointer-events-auto"
          >
            Continue Adventure
            <span className="ml-1 text-[24px] font-bold">›</span>
          </button>
        </div>
      </div>
    </div>
