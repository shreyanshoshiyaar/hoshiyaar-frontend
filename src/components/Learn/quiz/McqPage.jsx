            ...
            <div className="w-full max-w-md mx-auto">
                  ...
                  <div className="flex flex-wrap justify-center gap-3 mb-6">
                    {imgs.slice(0, 4).map((src, i) => (
                      <div key={i} className="bg-white rounded-2xl border border-gray-100 p-2 shadow-sm">
                        <img src={src} alt={'mcq-'+i} className="h-32 w-24 object-contain rounded-lg" />
                      </div>
                    ))}
                  </div>
                ) : null;
              ...
              <div className="grid grid-cols-1 gap-3">
                {item.options?.map((opt, idx) => {
                  ...
                  let buttonClass = "w-full p-4 rounded-2xl border-2 text-left transition-all duration-200 flex items-center justify-between ";
                  if (showResult) {
                    if (isSelected) {
                      buttonClass += isCorrect ? "bg-green-100 border-green-400 text-green-700" : "bg-red-100 border-red-400 text-red-700";
                    ...
                    >
                      {isImageUrl ? (
                        <div className="flex items-center gap-3">
                          <img src={opt} className="h-12 w-12 object-contain rounded-md" />
                          <span className="font-bold">Option {idx + 1}</span>
                        </div>
                      ) : (
                      ...
                      {showResult && (
                        <div className="flex-shrink-0">
                          {isSelected && (
                            <span className="text-xl font-black">{isCorrect ? '✓' : '✕'}</span>
                          )}
